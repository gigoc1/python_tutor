...
def moving_avg():
    jongmok_code = get_low_per_pbr(20221201)
...
def exam_func():
    gs = web.DataReader('095500', "naver", dt_previous)
    gs = gs.apply(to_numeric)
    ma5=gs['Volume'].rolling(window=5).mean()  #yahoo는 'Adj Close', naver는 'Close'
    ma20=gs['Volume'].rolling(window=20).mean()
    print(gs)
    print(gs['Volume'][-1]/ma5[-2])
exam_func()